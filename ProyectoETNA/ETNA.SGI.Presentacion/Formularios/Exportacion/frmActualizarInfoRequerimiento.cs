...
namespace ETNA.SGI.Presentacion.Formularios.Exportacion
{
    public partial class frmActualizarInfoRequerimiento : Form
    {
        ...
        private void frmActualizarInfoRequerimiento_Load(object sender, EventArgs e)
        {
            ...
            //objBus = new BTablas();
            dataGridView1.DataSource = FabricaNeg._instancia().ObtenerTablas().BRequerimientoBusquedaXCodigoCliente(Program.CodCli);
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            dataGridView1.GridColor = Color.Red;
            //objBus = new BTablas();
            dataGridView1.DataSource = FabricaNeg._instancia().ObtenerTablas().getSELECTLIBRE("SELECT Cod_Cab_Req,Fec_Esp_Cab_Req,(SELECT nom_pais FROM pais AS P WHERE P.Cod_Pais=R.Cod_Pais) AS pais,Destino_Cab_Req FROM Requerimiento AS R  " +
" WHERE COD_CLIENTE='" + Program.CodCli + "' AND EST_CAB_REQ<>'E' AND fec_reg_cab_req BETWEEN " + Desde + " AND " + hasta + "");
            ...
            if (e.ColumnIndex == 1)
            {
                try
                {
                    ...
                    if (MessageBox.Show("¿Desea Anular el Requerimiento Nro " + dataGridView1.Rows[p].Cells["codREQ"].Value.ToString() + " ?", "Exportación", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                    {
                        ...
                        int i = FabricaNeg._instancia().ObtenerTransaccion().BUpdateEstadoReq(dataGridView1.Rows[p].Cells["codREQ"].Value.ToString());
