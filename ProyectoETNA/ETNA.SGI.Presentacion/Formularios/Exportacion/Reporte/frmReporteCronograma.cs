...
namespace ETNA.SGI.Presentacion.Formularios.Exportacion.Reporte
{
    public partial class frmReporteCronograma : Form
    {
        ...
        private void frmReporteCronograma_Load(object sender, EventArgs e)
        {
            ...
            dt = FabricaNeg._instancia().ObtenerTablas().getSELECTLIBRE("SELECT cod_solicitud,Razon_Social,fec_esp_cab_req,D.idProducto,P.descripcionProducto,cantidad FROM SolicitudAtencion AS S LEFT OUTER JOIN " +
...
" WHERE S.Estado_Solicitud='X' and cod_solicitud in (" + codSolicitud + ")");
            ...
            eCronogramaDespachoBindingSource.DataSource = lst;
