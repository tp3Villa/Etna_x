...
namespace ETNA.SGI.Presentacion.Formularios.Exportacion
{
    public partial class frmCronogramaDespacho : Form
    {
        ...
        private void frmCronogramaDespacho_Load(object sender, EventArgs e)
        {
            ...
            dtSolicitud = FabricaNeg._instancia().ObtenerTablas().getSELECTLIBRE("SELECT cod_solicitud,S.Cod_Cab_Req AS cod_cab_req_solicitud,razon_social,Convert(varchar(10), fec_res_esp_solicitud,103) AS fec_res_esp_solicitud FROM SolicitudAtencion AS S LEFT OUTER JOIN " +
...
" cliente AS C ON (R.Cod_Cliente=C.Cod_Cliente) WHERE RAZON_SOCIAL LIKE '%" + razon + "%' AND fec_res_esp_solicitud BETWEEN '" + Desde + "' AND '" + hasta + "' and estado_solicitud='S'");
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                int K = FabricaNeg._instancia().ObtenerTransaccion().BTransaccionVarias("UPDATE dbo.SolicitudAtencion SET Estado_Solicitud='X' where Cod_Solicitud IN (" + condi + ")");
