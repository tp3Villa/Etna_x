    ...
    public partial class frmAprobacionDetaSolicitud : Form
    {
        ...
        private void frmAprobacionDetaSolicitud_Load(object sender, EventArgs e)
        {
            ...
            if (Opcion != "")
            {
                ...
                dt = FabricaNeg._instancia().ObtenerTablas().getSELECTLIBRE("SELECT             R.idProducto AS codProd,     P.descripcionProducto AS desProd,            cantidad AS cantidad, " +
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                try
                {
                    ...
                    //frm.ShowDialog();
                    //objBus = new BTablas();
                    dtReqProSii = FabricaNeg._instancia().ObtenerTablas().getSELECTLIBRE("SELECT R.cod_siicex,des_siicex,'Ver Doc' as DET FROM dbo.DocProdReq AS R left outer join " +
" dbo.DosSIICEX AS S on (R.cod_siicex=S.cod_siicex)  where r.Cod_Cab_Req= '" + CorrREQUPD + "' and r.idProducto='" + codPROD + "'");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                int K = FabricaNeg._instancia().ObtenerTransaccion().BTransaccionVarias("update dbo.SolicitudAtencion SET Estado_Solicitud='S' WHERE Cod_Solicitud='" + CorrSOLUPD + "'");
        ...
        private void dgvDocumento_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 3)
            {
                try
                {
                    ...
                    tblDocAdjunto = FabricaNeg._instancia().ObtenerTablas().getSELECTLIBRE("SELECT TOP 1 DOCUMENTO_ADJUNTO FROM DetalleDocExportacion WHERE Cod_SIICEX=" + CODPDF + " AND Estado='A' ORDER BY Version DESC");
